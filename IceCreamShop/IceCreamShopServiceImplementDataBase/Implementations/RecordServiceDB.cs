    ...
    public class RecordServiceDB : IRecordService
    {
        ...
        public void SaveIceCreamPrice(RecordBindingModel model)
        {
            ...
            try
            {
                ...
                var table = document.Tables.Add(rangeTable, icecreams.Count, 2, ref missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = icecreams[i].IceCreamName;
                    table.Cell(i + 1, 2).Range.Text = icecreams[i].Price.ToString();
                }
        ...
        public List<StoragesLoadViewModel> GetStoragesLoad()
        {
            return context.Storages
                            .ToList()
                            .GroupJoin(
                                context.StorageIngredients
                                    .Include(r => r.Ingredient)
                                    .ToList(),
                                storage => storage,
                                storageIngredient => storageIngredient.Storage,
                                (storage, storageCompList) =>
            new StoragesLoadViewModel
            {
                StorageName = storage.StorageName,
                TotalCount = storageCompList.Sum(r => r.Count),
                Ingredients = storageCompList.Select(r => new Tuple<string,
        ...
        public void SaveStoragesLoad(RecordBindingModel model)
        {
            ...
            try
            {
                ...
                excelcells.Value2 = "Загруженность хранилищ";
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        if (elem.Ingredients.Count() > 0)
                        {
                            ...
                            excelworksheet.get_Range(excelcells,
                                        excelcells.get_Offset(elem.Ingredients.Count() - 1, 1));
        ...
        public void SaveCustomerBookings(RecordBindingModel model)
        {
            ...
            //вставляем заголовок
            var phraseTitle = new Phrase("Заказы покупателей",
            ...
            table.AddCell(new PdfPCell(new Phrase("ФИО покупателя", fontForCellBold))
            {
            ...
            table.AddCell(new PdfPCell(new Phrase("Мороженое", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].IceCreamName, fontForCells));
                table.AddCell(cell);
